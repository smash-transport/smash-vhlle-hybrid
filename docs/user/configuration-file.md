...
???+ config-key "`Input_file`"
    ...
    However, using this key, any file can be specified and used.
    Note that although it is possible to specify the input for the List Modus in SMASH via the `Software_keys`, this is not allowed here 
    and will throw an error. Always specify the input to the Afterburner stage here, if needed.
