...
???+ config-key "`Input_file`"
    ...
    If the key is a simple file name (without any `/`), the hybrid handler looks for this name in the corresponding :file_folder: ***Sampler*** output sub-folder, but if it is a path (i.e. it contains a `/`), that specific file will be used.
