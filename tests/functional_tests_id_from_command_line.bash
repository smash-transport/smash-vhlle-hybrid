...
function Functional_Test__do-IC-only-with-ID-from-command-line()
{
    ...
    if [[ $? -ne 0 ]]; then
        ...
        return 1
    elif [[ $(yq '.Hybrid_handler.Run_ID' "$(pwd)/IC/${run_id}/Hybrid_handler_IC_config.yaml") != "${run_id}" ]]; then
        Print_Error 'Hybrid-handler failed to use command line ID into stored configuration.'
        return 1
    fi
