...
# README:
# The fist sampler black box is called like:
#
#     ./fist_sampler_black_box path_to_config_file
#
# with:
...
def check_input_arguments():
    calling_instruction = 'Call the sampler black box by:\n\n' +\
                          './fist_sampler_black_box.py events num path_to_config_file \n\n' +\
                          '- path_to_config_file: Path to the sampler configuration\n'
...
def check_if_file_exists(path, type):
    if type=="hypersurface":
        err_msg = 'Hypersurface file  ' + path + ' not found! \n Empty hypersurface! Aborting...'
    elif type=="decays":
        ...
        err_msg = 'Hypersurface file surface_eps_0.26.dat not found!'
...
if __name__=='__main__':
    ...
    check_if_file_exists(path_to_config, "config")

    path_to_freezeout = get_value_as_string_from_config_by_keyword(path_to_config, 'hypersurface_file')
    output_file = get_value_as_string_from_config_by_keyword(path_to_config, 'output_file')
    decays_file = get_value_as_string_from_config_by_keyword(path_to_config, 'decays_list_file')
    particle_file = get_value_as_string_from_config_by_keyword(path_to_config, 'particle_list_file')


    check_if_file_exists(path_to_freezeout, "hypersurface")
    check_if_file_exists(decays_file, "decays")
    check_if_file_exists(particle_file, "particle")
