...
function Unit_Test__Afterburner-create-input-file()
{
    ...
    local -r \
        ...
        plist_Final="${HYBRID_software_output_directory[Afterburner]}/sampled_particles_list.oscar"
    touch "${plist_Sampler}"
...
function Unit_Test__Afterburner-check-all-input()
{
    ...
    touch \
        "${HYBRID_software_output_directory[Afterburner]}/sampled_particles_list.oscar" \
    ...
    rm "${HYBRID_software_output_directory[Afterburner]}/sampled_particles_list.oscar"
    touch "${HYBRID_software_output_directory[Sampler]}/original_sampled_particles_list.oscar"
    ln -s "${HYBRID_software_output_directory[Sampler]}/original_sampled_particles_list.oscar" \
        "${HYBRID_software_output_directory[Afterburner]}/sampled_particles_list.oscar"
    Call_Codebase_Function_In_Subshell Ensure_All_Needed_Input_Exists_Afterburner &> /dev/null
    if [[ $? -ne 0 ]]; then
    ...
    rm "${HYBRID_software_output_directory[Sampler]}/original_sampled_particles_list.oscar"
    Call_Codebase_Function_In_Subshell Ensure_All_Needed_Input_Exists_Afterburner &> /dev/null
    if [[ $? -eq 0 ]]; then
